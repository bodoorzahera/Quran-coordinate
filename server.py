...
<title>محرر إحداثيات القرآن</title>
...
<div id="toolbar">
 ...
 <div class="g">
  ...
  <button class="btn" onclick="zoomFit()">ملائمة</button>
  <span class="lbl" id="zLbl">100%</span>
 </div><div class="sep"></div>
 <div class="g">
  <button class="btn on" id="bxBtn" onclick="togBoxes()">إطارات</button>
  <button class="btn on" id="lblBtn" onclick="togLabels()">كلمات</button>
 </div><div class="sep"></div>
 <div class="g">
  <button class="btn lock locked" id="lockBtn" onclick="togLock()">🔒</button>
  <button class="btn" id="addBtn" onclick="togAdd()" style="display:none">+ إضافة</button>
  <button class="btn red" id="delBtn" onclick="delSel()" style="display:none">🗑</button>
 </div><div class="sep"></div>
 <div class="g">
  <button class="btn grn" onclick="saveAll()">💾</button>
...
<div id="infoP">
 <div class="row">
  ...
  <div class="fd"><label style="width:auto">الموقع</label><input class="li" id="locIn" dir="ltr"></div>
  <button class="ibtn" onclick="applyLoc()" style="background:var(--bl)">تطبيق</button>
  <button class="ibtn" onclick="desel()" style="background:var(--sf2);color:var(--tx)">✕</button>
 </div>
 <div class="row" id="editRow">
  <div class="fd"><label>X</label>
   <div class="ar ll" data-f="cX" data-d="-10">»</div><div class="ar" data-f="cX" data-d="-1">›</div>
   <input type="number" id="cX">
   <div class="ar" data-f="cX" data-d="1">‹</div><div class="ar rr" data-f="cX" data-d="10">«</div>
  </div>
  <div class="fd"><label>Y</label>
   <div class="ar ll" data-f="cY" data-d="-10">»</div><div class="ar" data-f="cY" data-d="-1">›</div>
   <input type="number" id="cY">
   <div class="ar" data-f="cY" data-d="1">‹</div><div class="ar rr" data-f="cY" data-d="10">«</div>
  </div>
  <div class="fd"><label>W</label>
   <div class="ar ll" data-f="cW" data-d="-10">»</div><div class="ar" data-f="cW" data-d="-1">›</div>
   <input type="number" id="cW">
   <div class="ar" data-f="cW" data-d="1">‹</div><div class="ar rr" data-f="cW" data-d="10">«</div>
  </div>
  <div class="fd"><label>H</label>
   <div class="ar ll" data-f="cH" data-d="-10">»</div><div class="ar" data-f="cH" data-d="-1">›</div>
   <input type="number" id="cH">
   <div class="ar" data-f="cH" data-d="1">‹</div><div class="ar rr" data-f="cH" data-d="10">«</div>
  </div>
 </div>
</div>
...
function togTheme(){
  darkMode=!darkMode;
  if(darkMode){document.documentElement.removeAttribute('data-theme');$('themeBtn').textContent='☀️';localStorage.setItem('theme','dark');}
  else{document.documentElement.dataset.theme='light';$('themeBtn').textContent='🌙';localStorage.setItem('theme','light');}
}
...
function applyUi(){
  ...
  $('toolbar').style.padding=(8*s)+'px '+(12*s)+'px';
  ...
  requestAnimationFrame(()=>{
    $('vp').style.top=$('toolbar').offsetHeight+'px';
  });
}
...
function togLock(){
  ...
  if(editLocked){b.textContent='🔒';b.className='btn lock locked';$('addBtn').style.display='none';$('delBtn').style.display='none';$('editRow').style.display='none';}
  else{b.textContent='🔓';b.className='btn lock unlocked';$('addBtn').style.display='';$('delBtn').style.display='';if(selLoc)$('editRow').style.display='';}
  render();
}
...
// ─── Edit ───
function togAdd(){if(editLocked)return;addMode=!addMode;$('addBtn').classList.toggle('on',addMode);$('addBtn').textContent=addMode?'❌ إلغاء':'+ إضافة';$('ov').style.cursor=addMode?'crosshair':'';}
...
function delSel(){if(editLocked||!selLoc){toast('حدد إطاراً',1);return;}if(!confirm(`حذف ${selLoc}؟`))return;delete coords[selLoc];dirty=true;selLoc=null;$('infoP').classList.remove('vis');render();updateStats();toast('تم الحذف');}
...
function showInfo(l){
  ...
  const wd=$('wordDisp');
  if(mushaf[l]){wd.textContent=mushaf[l].word;wd.style.color='var(--gd)';}else{wd.textContent='⚠ لا كلمة';wd.style.color='#f87171';}
  $('editRow').style.display=editLocked?'none':'';
  $('infoP').classList.add('vis');
}
function applyLoc(){
...
function onDown(e){
  ...
  if(hit){
    selLoc=hit;showInfo(hit);
    if(!editLocked){
      interaction={type:'drag',loc:hit,spx:nat.px,spy:nat.py,orig:{...(coords[hit].h||coords[hit])}};
    }
    render();
    return;
  }
